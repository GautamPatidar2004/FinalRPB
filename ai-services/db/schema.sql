...
-- Operational Read Policies (Corridors, Assets, Trains): Read accessible by authenticated and anon service
CREATE POLICY "Operational corridors viewable by all users"
    ON corridors FOR SELECT TO authenticated, anon
    USING (TRUE);

CREATE POLICY "Operational assets viewable by all users"
    ON assets FOR SELECT TO authenticated, anon
    USING (TRUE);

CREATE POLICY "Operational trains viewable by all users"
    ON trains FOR SELECT TO authenticated, anon
    USING (TRUE);

-- Operational Write Policies (Service role or Admin)
...
CREATE POLICY "View block requests"
    ON maintenance_block_requests FOR SELECT TO authenticated, anon
    USING (TRUE);

CREATE POLICY "Create block requests"
    ON maintenance_block_requests FOR INSERT TO authenticated
    WITH CHECK (TRUE);

CREATE POLICY "Update block requests"
    ON maintenance_block_requests FOR UPDATE TO authenticated
    USING (TRUE);

-- Block Plans and Items Policies
CREATE POLICY "View block plans"
    ON block_plans FOR SELECT TO authenticated, anon
    USING (TRUE);

CREATE POLICY "Manage block plans"
    ON block_plans FOR ALL TO authenticated
    USING (TRUE);

CREATE POLICY "View plan items"
    ON block_plan_items FOR SELECT TO authenticated, anon
    USING (TRUE);

CREATE POLICY "Manage plan items"
    ON block_plan_items FOR ALL TO authenticated
    USING (TRUE);
